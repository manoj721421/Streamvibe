...
const TVShowsCards = ({ movieList, head, subHead, pagination, cardType }) => {
    ...
    const currentmovies = movieList?.slice(indexOfFirstGenre, indexOfLastGenre);
    return (
        <>
                    ...
                    <Row className={`mt-5 ${isMobile && "genresCard"}`}>
                        {
                            ...
                            currentmovies?.map((item, index) => {
                                return (
                                    <>
                                        <Col md={3} sm={6} xs={9}>
                                            <Card className='background-dark ' style={{ width: '18rem', borderRadius: "1em" }} onClick={()=>{navigate(`/series/${item?.id}`)}}>
